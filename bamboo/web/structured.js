...
function paintStructured(){
 ...
 for(const [index,record]of structuredDraft.records.entries()){
  ...
  for(const f of fields){const cell=document.createElement('td'),input=document.createElement(f.multiline?'textarea':'input');input.dataset.field=f.key;input.value=record[f.key]??f.value??'';input.setAttribute('aria-label',f.label+' '+(index+1));if(f.required)input.required=true;if(input.tagName==='INPUT')input.type=f.type||'text';if(f.key==='amount')input.inputMode='decimal';input.oninput=()=>{record[f.key]=input.value;moneyPreview();};cell.append(input);row.append(cell);}
  const actions=document.createElement('td');
  for(const [label,delta]of [['上移',-1],['下移',1],['删除',0]]){const button=document.createElement('button');button.type='button';button.textContent=label;button.disabled=delta===-1&&index===0||delta===1&&index===structuredDraft.records.length-1;button.onclick=()=>{if(delta){const records=structuredDraft.records;[records[index],records[index+delta]]=[records[index+delta],records[index]];}else structuredDraft.records.splice(index,1);paintStructured();};actions.append(button);}
...
$('structured-add').onclick=()=>{const record={id:crypto.randomUUID().replaceAll('-','')};for(const f of structuredFields(structuredDraft.kind))record[f.key]=f.value||'';structuredDraft.records.push(record);paintStructured();$('structured-records').querySelector('tbody tr:last-child input')?.focus();};
