...
| `exporters/wordflow.py` | 原生 Word 正文、网格、双行合一、课注、ruby、脚注和批注框 |
...
`Block` 区分正文、题名、课注、分页符，保留缩进和层级。`Inline` 区分普通文字、重点、夹注、短旁注和脚注。短旁注的 `text` 是基文，`annotation` 是注文。`Annotation` 保存长旁批或眉批的源锚点与区域容量。
...
双行夹注占用主文字流中的空间，使用半字号的两条小字流，跨栏续排并平衡末段。课注另起小字号段落，与前段关联。短旁注使用基文与注文配对，在 Word 中以 ruby 表达。
...
- 夹注使用 `w:eastAsianLayout combine`，短旁注使用 `w:ruby`，课注使用独立样式与 `keepNext`。
