...
完整检查共 164 个用例，包含 9 个真实浏览器测试。新增检查覆盖独立元素开关、24 种方向与样式组合、编号引用重编号、标签边框、Word 域结构及导入回读。原有检查包含 40 组固定种子随机混合文档。新增检查覆盖事务原子性、稳定段落标识、Unicode 删除、锚点移动、撤销重做、选区格式、保存恢复及 Word 实际内容导入。
