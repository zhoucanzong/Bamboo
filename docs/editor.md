...
`Book.special` 可保存 `GiftLedger`，每条 `GiftRecord` 有稳定标识及姓名、金额、礼品、日期、备注。`set_special` 在一个事务中保存整组记录，几何校验成功后才提交，因此失败不会覆盖原文档。普通文档的段落编辑命令不会用于礼簿。前端使用可直接填写的记录表格，支持纸面点击定位、增删排序、合计预览和撤销。
