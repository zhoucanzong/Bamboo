...
界面包含新建、打开、保存副本、导出、撤销重做、题名与段后注样式、夹注、短旁注、脚注、独立旁批／眉批、横竖方向和版式参数。失败的文字提交会显示可恢复文本，避免输入被错误提示吞掉。
...
当前 Word 导入支持普通段落、分级题名、双行夹注、ruby 和脚注。复杂表格转为行文字，浮动批注文字转为随段段后注，图片未转成可编辑对象；这些差异会在界面说明。完整 Word 兼容不是本版本的承诺。
...
`Book.special` 可保存 `GiftLedger`，每条 `GiftRecord` 有稳定标识及姓名、金额、物品、日期、备注。`set_special` 在一个事务中保存整组记录，几何校验成功后才提交，因此失败不会覆盖原文档。普通文档的段落编辑命令不会用于册簿。前端使用可直接填写的记录表格，支持纸面点击定位、增删排序、合计预览和撤销。

金额始终存为两位小数的字符串，以 `Decimal` 计算；拒绝负数、科学计数法和超过两位的小数。Word 的记录单元格带有不可见字段标识，回导按实际字段值恢复并重新计算合计。旧的 `commentary` 类型及样式 key 保持兼容，界面中文名称改为“段后注”。


## 族谱与工尺谱编辑

`Genealogy.records` 保存 `FamilyPerson`。父母、配偶通过稳定人物 ID 关联，配偶关系规范化为双向；折叠配偶组后对亲子图拓扑排序，计算世代并拒绝循环。世系图按世代排序后分页，页外亲属标注对应页码；每个人物另附传记页。Word 的图框姓名与传记姓名按实际编辑合并，冲突时回退为实际文字并提示核对。

`GongcheScore.records` 保存有序 `GongcheNote`，包含谱字、板眼、音区、分句、唱词与 lyric_span。跨度不能越过分句、覆盖另一条唱词或超过分组容量。分组时不会拆散一字多音；编辑界面按对应组整体移动，删除首音时将唱词交给剩余对应音，删除组内音时缩短跨度。

工尺谱 Word 导出使用原生嵌套表格、横向或纵向合并；回读真实谱字、唱词、分句和合并跨度。重复字段标识或破坏结构的第三方改动会触发普通文字回退，不使用旧快照覆盖真实文字。所有专用数据均可保存副本、撤销与重新打开。


## 字体和可编辑朱批

字体选项包含可用的系统宋体、楷体及本机缓存中的霞鹜文楷。Book.font 保存选择，文档预览与三种导出共用字体；缺少所选字体时明确提示回退。字体获取操作为显式下载，版本和 SHA-256 固定，不上传用户文档。

`update_annotation` 可以修改现有批注的文字、位置类型、颜色、字号比例、小栏数和续排设置，原始文字锚点保持不变。纸面上的红色批注和侧栏编辑按钮都可打开设置。“对页预览”仅改变显示方式，不改变文档分页，竖排时第一页面位于右侧。

自动续排使用独立原文偏移检查每段批注的字数守恒。Word 按当前排版结果生成可编辑片段；精确页内布局与 Word 内部的动态分页不同，Word 改字后需重新从简牍原稿导出。
